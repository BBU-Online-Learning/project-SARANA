...
function connect(index) {
    const ws = new WebSocket(`ws://127.0.0.1:${process.env.CLASS_TEST_PORT}/app/${process.env.CLASS_TEST_KEY}?protocol=7&client=js&version=8.4.0&flash=false`);
    sockets[index] = ws;
    ws.addEventListener('message', event => {
        ...
        if (packet.event === 'pusher:connection_established') {
            ...
            throw Error('Reverb rejected the test subscription');
        }
    });
