...
test('shared navigation has permitted real links and no excluded template controls', function (string $role): void {
    ...
    $this->get(route('chat.index'))->assertSee('data-chat-menu', false)->assertSee('data-chat-list', false)->assertSee('chat-load-status');
})->with(Role::NAMES);
...
test('all four roles may update only their own personal details', function (string $role): void {
    ...
    $this->actingAs($user)->patch(route('profile.update'), ['name' => '  New Name  ', 'phone' => '+855 (12) 345-678'])
        ->assertRedirect(route('profile.edit'))->assertSessionHas('success', 'Profile updated.');
...
test('profile photos use fake storage safe names and never delete existing images', function (): void {
    ...
    Storage::disk('public')->assertExists('images/users/old.png');
    expect(Storage::disk('public')->allFiles('images/users'))->toHaveCount(2);
